...
Before starting ComfyStream UI, we must start the ComfyStream API server: <Note>This should be on a system with a GPU</Note>
...
ComfyStream UI must be ran on a system with physical or virtual camera. In this guide we will install on a local machine
