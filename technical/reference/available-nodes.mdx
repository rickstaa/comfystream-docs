...
<AccordionGroup>
  <Accordion header="Depth Anything TensorRT" title="Depth Anything TensorRT">
    <Tabs>
      ...
      <CodeGroup>
        ...
        ```bash Local (Windows)
        mkdir -p "$env:HOMEPATH/comfyRealtime/ComfyUI/models/tensorrt/depth-anything"
        mv depth_anything_vitl14-fp16.engine "$env:HOMEPATH/comfyRealtime/ComfyUI/models/tensorrt/depth-anything/"
        ```
      </CodeGroup>
  ...
  <Accordion header="Segment Anything 2" title="Segment Anything 2">
    <Tabs>
      ...
      <Note>
      For Windows, ensure the prerequisites are installed by following the Windows section in [Install Nodes in ComfyUI](../../reference/custom-node-installation##install-nodes-in-comfyui)
      </Note>
