...
ComfyStream is available as a custom node through the [comfyui manager](https://registry.comfy.org/nodes/comfystream) and also as a docker image [livepeer/comfystream:stable](https://hub.docker.com/r/livepeer/comfystream/tags?name=stable) choose whichever best fits your needs.

<Tabs>
  <Tab title="Docker Image">
    ## Installing with Docker
    
    Docker provides an easy way to get started with ComfyStream with all dependencies pre-configured.
    ...
    ComfyStream can be deployed on any cloud provider with GPU support. Documentation includes multiple options for remote deployment:
      ...
      <AccordionGroup>
        <Accordion title="Deploy using Ansible Playbooks">
          ...
          ```
            ...
            <Step title="Access the Server">
              ...
              ```
              <Note>
              Accessing comfyUI in ansible deployed server will be proxied through a different port. Make sure you are accessing the correct port **8189**.
              </Note>
            </Step>
          </Steps>
        </Accordion>
        ...
        <Accordion title="Deploy on RunPod">
          <Steps>
            ...
            <Step title="Connect to pod dashboard">
              Once the container is fully running, you can access ComfyUI by clicking **Connect** in the pod dashboard.
              ![comfyui-connect-pod](../../images/comfyui-connect-pod.png)
            </Step>

            <Step title="Access ComfyUI">
              Access ComfyUI at **HTTP Service -> :8188**. It will open in a new tab
            </Step>
          </Steps>
        </Accordion>
      </AccordionGroup>
    </Tab>
    <Tab title="Custom Node">
    ## Comfystream as a Custom Node

    If you already have ComfyUI installed, you can install ComfyStream as a custom node without any further setup.
      ...
      <Step title="Choose Installation Method">
        <Tabs>
          <Tab title="ComfyUI Manager">
            1. Install [ComfyUI Manager](https://github.com/Comfy-Org/ComfyUI-Manager) following the [installation guide](https://docs.comfy.org/essentials/core-concepts/nodes#installing-the-manager).
            2. Open ComfyUI and click on the "Manager" tab
...
<AccordionGroup>
  <Accordion title="Docker Hub Rate Limits (toomanyrequests error)">
    <Note>
      If you encounter a `toomanyrequests` error while pulling the Docker image,
      either wait a few minutes or provide your Docker credentials when running
      the playbook: ```bash ansible-playbook -i inventory.yaml
      plays/setup_comfystream.yaml -e
      "docker_hub_username=your_dockerhub_username
      docker_hub_password=your_dockerhub_pat" ```
    </Note>
  </Accordion>
</AccordionGroup>
