...
```
<Note>If you are copying files from Windows desktop to WSL, you will not be able to find the directory `/workspace/comfyRealtime/ComfyUI/models/tensorrt/depth-anything/`. Instead, use `/workspace/ComfyUI/models/tensorrt/depth-anything/`</Note>
...
6. In the **Load Diffusion Model** node: 
    - Select *dreamshaper_8_dmd_1kstep.safetensors* file from the `unet` folder. 
...
3. Once ComfyUI has started, download the **TAESD Encoder** and **TAESD Encoder**
    - From **ComfyUI Manager** > **Model Manager**, search `TAESD`
    - Download **TAESD Encoder** and **TAESD Decoder** ![TAESD](../../images/TAESD.png)
